    ...
    <Screen>
      <View className="mb-5 flex-row items-end justify-between">
        <View>
          ...
          <Text className="mt-1 font-body text-sm text-muted">{getTodayLabel()}</Text>
        </View>
        <View className="flex-row items-center rounded-full border border-xp/20 bg-xp/10 px-3 py-2">
          ...
          <Text className="ml-1.5 font-headline text-xs text-foreground">Constrói o teu ritmo</Text>
        </View>
      </View>
      ...
      {persistenceError ? (
        <View className="mt-4 rounded-2xl border border-[#FB7185]/40 bg-[#FB7185]/10 px-4 py-3">
          <Text className="font-body text-sm leading-5 text-[#FDA4AF]">{persistenceError}</Text>
        </View>
      ) : null}
