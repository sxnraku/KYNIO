...
import { fasts, friends, meals, userProfile, workouts } from '@/db/schema';
...
export async function deleteRemoteFriendContact(
  ...
  requireNoError(result.error, 'Não foi possível remover o amigo da sincronização.');
}
...
function requireNoError(error: { message: string } | null, fallback: string): void {
  if (error) {
...
async function createSignedAvatarUrl(path: string): Promise<string> {
  ...
  requireNoError(result.error, 'Não foi possível carregar a fotografia de perfil remota.');

  if (!result.data?.signedUrl) {
    throw new Error('A fotografia de perfil remota não devolveu um endereço válido.');
  }
...
async function uploadLocalAvatar(
  ...
  if (!response.ok) {
    throw new Error('Não foi possível preparar a fotografia de perfil para sincronização.');
  }
  ...
  requireNoError(upload.error, 'Não foi possível sincronizar a fotografia de perfil.');
  return path;
}
...
export async function syncAllUserData(): Promise<CloudSyncResult> {
  ...
  const remoteProfileResult = await client
    ...
    .eq('user_id', user.id)
    .maybeSingle();
  requireNoError(remoteProfileResult.error, 'Não foi possível carregar o perfil remoto.');
  const remoteProfile = remoteProfileResult.data as unknown as RemoteProfileRow | null;

  if (remoteProfile && Number(remoteProfile.updated_at) > localProfile.profileUpdatedAt) {
    ...
    const hasCustomLocalAvatar = Boolean(
      localProfile.avatarUri && localProfile.avatarUri !== localProfile.googleAvatarUrl,
    );
    ...
    if (hasCustomLocalAvatar && localProfile.avatarUri && !avatarPath) {
      ...
      const removal = await client.storage.from('profile-avatars').remove([avatarPath]);
      requireNoError(removal.error, 'Não foi possível remover a fotografia de perfil remota.');
      avatarPath = null;
      const [updatedProfile] = await database
        .update(userProfile)
    ...
    requireNoError(profileUpload.error, 'Não foi possível guardar o perfil remoto.');
  }

  const [localFasts, localMeals, localWorkouts, localFriends] = await Promise.all([
  ...
  const uploads = [
    fastUploads.length
      ? client.from('fasts').upsert(fastUploads, { onConflict: 'user_id,record_key' })
      : Promise.resolve({ error: null }),
    mealUploads.length
      ? client.from('meals').upsert(mealUploads, { onConflict: 'user_id,record_key' })
      : Promise.resolve({ error: null }),
    workoutUploads.length
      ? client.from('workouts').upsert(workoutUploads, { onConflict: 'user_id,record_key' })
      : Promise.resolve({ error: null }),
  ...
  const [fastDownload, mealDownload, workoutDownload, friendDownload] = await Promise.all([
  ...
  requireNoError(workoutDownload.error, 'Não foi possível descarregar atividades.');
  ...
  const remoteWorkouts = (workoutDownload.data ?? []) as unknown as RemoteWorkoutRow[];
  const remoteFriends = (friendDownload.data ?? []) as unknown as RemoteFriendRow[];
  const localFastKeys = new Set(localFasts.map((fast) => fastKey(fast.startTime, fast.endTime)));
  const localMealKeys = new Set(localMeals.map((meal) => mealKey(meal.timestamp)));
  const localWorkoutKeys = new Set(localWorkouts.map((workout) => workoutKey(workout.timestamp)));
  const localFriendKeys = new Set(
    localFriends.map((friend) => friendKey(friend.createdAt, friend.displayName)),
  );
  const missingFasts = remoteFasts.filter((fast) => !localFastKeys.has(fast.record_key));
  const missingMeals = remoteMeals.filter((meal) => !localMealKeys.has(meal.record_key));
  ...
  await database
    .update(userProfile)
    .set({ cloudUserId: user.id, currentLevel: calculateLevel(totalXp), totalXp })
    .where(eq(userProfile.id, 1));
  ...
  if (profile) {
    ...
    requireNoError(finalProfileUpload.error, 'Não foi possível finalizar a sincronização.');
  }

  return {
    downloadedRecords:
      missingFasts.length + missingMeals.length + missingWorkouts.length + missingFriends.length,
    syncedAt: syncTimestamp,
    uploadedRecords:
      fastUploads.length + mealUploads.length + workoutUploads.length + friendUploads.length,
  };
}
