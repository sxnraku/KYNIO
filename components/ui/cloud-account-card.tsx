...
export function CloudAccountCard({ onLocalDataChanged }: CloudAccountCardProps) {
    ...
    <Card>
      <View className="flex-row items-start gap-3">
        ...
        <View className="min-w-0 flex-1">
          <Text className="font-headline text-lg text-foreground">Conta e sincronização</Text>
      ...
      {!account.isConfigured ? (
        <View className="mt-5 rounded-xl border border-xp/20 bg-xp/5 p-4">
          <Text className="font-headline text-sm text-foreground">Configuração necessária</Text>
          <Text className="mt-2 font-body text-xs leading-5 text-muted">
            Adiciona EXPO_PUBLIC_SUPABASE_URL e EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY ao ficheiro
            .env.local para ativar o login Google.
          </Text>
        </View>
      ) : account.account ? (
        <View className="mt-5">
          <View className="flex-row items-center rounded-xl border border-border bg-background p-4">
            ...
            <View className="ml-3 min-w-0 flex-1">
              ...
              <Text className="mt-0.5 font-body text-xs text-muted" numberOfLines={1}>
          ...
          <Pressable
            ...
            <Text className="font-body text-sm text-muted">Desligar conta Google</Text>
          </Pressable>
        </View>
      ) : (
        <View className="mt-5">
          <View className="rounded-xl border border-border bg-background p-4">
            <View className="flex-row items-start gap-2">
              ...
              <Text className="flex-1 font-body text-xs leading-5 text-muted">
                Ao ligar a conta, nome, email, perfil, amigos e registos de hábitos são guardados
                remotamente para sincronização. A ligação é opcional e pode ser removida.
              </Text>
            </View>
          </View>
          ...
          <Pressable
            ...
            <Text className="font-headline text-sm text-foreground">Continuar com Google</Text>
          </Pressable>
        </View>
      )}

      {account.error ? (
        <Text className="mt-4 font-body text-xs leading-5 text-red-500">{account.error}</Text>
      ) : null}
      {account.message ? (
        <Text className="mt-4 font-body text-xs leading-5 text-success">{account.message}</Text>
      ) : null}
    </Card>
  );
}
