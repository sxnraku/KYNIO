...
export function FriendsPanel({
  ...
  const confirmRemove = (friend: FriendRecord) => {
    Alert.alert(
      translateText("Remover amigo?", language),
      language === "en"
        ? `${friend.displayName} will be removed from your list and the change will sync the next time you connect.`
      ...
      [
        { style: "cancel", text: translateText("Cancelar", language) },
        {
          ...
          style: "destructive",
          text: translateText("Remover", language),
        },
      ],
    );
  };
    ...
    <View>
      <Card>
        ...
        <Text className="mt-2 font-body text-sm leading-5 text-muted">
          Guarda pessoas importantes e sincroniza a lista entre os teus
          dispositivos. Não são partilhados jejuns, refeições ou treinos
          automaticamente.
        </Text>
