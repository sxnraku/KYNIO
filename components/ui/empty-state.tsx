...
export function EmptyState({ description, icon, title }: EmptyStateProps) {
  return (
    <View className="items-center rounded-2xl border border-dashed border-border bg-surface px-6 py-10">
      ...
      <Text className="mt-4 text-center font-headline text-lg text-foreground">{title}</Text>
      <Text className="mt-2 text-center font-body text-sm leading-5 text-muted">{description}</Text>
    </View>
  );
}
