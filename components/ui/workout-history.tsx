      ...
      <Card>
        <View className="items-center py-5">
          <View className="h-14 w-14 items-center justify-center rounded-full bg-success-dark">
            <Ionicons color={COLORS.success} name="footsteps-outline" size={25} />
          </View>
          <Text className="mt-4 font-headline text-base text-foreground">Ainda sem atividades</Text>
  ...
  return (
    <Card>
      {records.slice(0, 5).map((record, index) => {
        const label = WORKOUT_OPTIONS.find((option) => option.id === record.type)?.label ?? 'Atividade';
        ...
        return (
          <View
            ...
            <View className="h-11 w-11 items-center justify-center rounded-2xl bg-success-dark">
              <Ionicons color={COLORS.success} name={ICONS[record.type] ?? 'fitness-outline'} size={21} />
            </View>
            ...
            <View className="items-end">
              <Text className="font-label text-[10px] text-xp">+{record.xpEarned} XP</Text>
              <Text className="mt-1 font-body text-[11px] text-muted">{formatDate(record.timestamp)}</Text>
            </View>
          </View>
        );
      })}
