...
export function MetabolicPhases({ elapsedHours, isActive }: MetabolicPhasesProps) {
    ...
    <View className="mt-8">
      <View className="mb-4 flex-row items-end justify-between">
        <View className="flex-1 pr-4">
          <Text className="font-headline text-2xl text-foreground">A tua jornada</Text>
          <Text className="mt-1 font-body text-sm text-muted">Fases metabólicas estimadas</Text>
        </View>
      ...
      <ScrollView
        ...
        {ESTIMATED_METABOLIC_PHASES.map((phase, index) => {
          ...
          const accentColor = isCurrent || isCompleted ? COLORS.success : COLORS.muted;
          ...
          return (
            <View
              ...
              key={phase.timeRange}
              style={{
                backgroundColor: isCurrent ? 'rgba(16, 185, 129, 0.08)' : COLORS.surface,
              ...
              <View className="flex-row items-center justify-between">
                ...
                {isCurrent ? (
                  <Text className="font-label text-[9px] text-success">ATUAL</Text>
                ) : null}
              </View>
              <Text className="mt-4 font-body text-xs text-muted">{phase.timeRange}</Text>
